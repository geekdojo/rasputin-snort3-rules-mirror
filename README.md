...
Three details are there because simpler versions were tried and do not work:

- `snort-mgr check` **without `-v` skips the rules** and passes even with a broken
...
- **A functional test**, [`tests/functional-snort.sh`](tests/functional-snort.sh). It
  ...
  regular expression, and a duplicate rule id that Snort silently drops.
